...
const DEFAULT_GATEWAYS = {
  ...
  flevopay: {
    ...
    statusText: 'Configurado e disponível',
    publicKey: '',
    // Baked as a fixed fallback so every instance works even without the env var set
    // on Vercel - this key is not meant to change.
    secretKey: process.env.FLEVO_API_KEY || process.env.FLEVO_SECRET_KEY || 'sk_cead2f108372da0e04bedb8bd08e4f6035ae815472af580737b47b5b77da2cdd',
    maxAmountCents: 100000
  },
  pingupag: {
    key: 'pingupag',
...
// Fixed admin credentials, read directly from env/constant rather than from db.config.
// A serverless instance can be warm with an old cached db.json (e.g. from before the
// password was pinned to these values) - if session signing/validation read the
// mutable db.config copy instead, two instances disagreeing on that value would sign
// and verify with different secrets, rejecting perfectly valid tokens as "expired".
const ADMIN_USER_FIXED = process.env.ADMIN_USER || 'Desenrola.2026';
const ADMIN_PASSWORD_FIXED = process.env.ADMIN_PASSWORD || 'Dubai.2026';

function getInitialDB() {
  const adminUser = ADMIN_USER_FIXED;
  const adminPassword = ADMIN_PASSWORD_FIXED;
  return {
    config: {
...
function getSessionSecret() {
  return crypto.createHash('sha256').update('recupera_session_v1:' + ADMIN_USER_FIXED + ':' + ADMIN_PASSWORD_FIXED).digest('hex');
}
...
function loginAdmin(username, password) {
  if (username === ADMIN_USER_FIXED && password === ADMIN_PASSWORD_FIXED) {
...
function validateSession(token) {
  ...
  if (payload.username !== ADMIN_USER_FIXED) return null;
...
async function createFlevoPayTransaction({ amount, name, cpf, phone, email, title, tracking }) {
  ...
  const payload = {
    amount: amountCents,
    description: title || 'Curso MoneyClub',
...
// Vercel serverless instances don't share the local orders file with each
// other, so an order created on one instance can be invisible to the instance
// that later serves the admin panel's poll. FlevoPay's own API is a real,
// shared, persistent backend that already has every transaction regardless of
// which instance created it - so when it's the active gateway, we use it as
// the source of truth for the orders list instead of trusting only local
// storage. customer_data is only returned by the single-transaction lookup,
// not the list, and it never changes once created, so it's cached in-memory
// per-instance to avoid an extra API call on every single poll.
const flevoCustomerCache = {};

async function fetchFlevoPayTransactionsList(limit = 50) {
  const apiKey = getFlevoApiKey();
  if (!apiKey) return [];
  try {
    const res = await fetch(`${FLEVO_BASE_URL}/api/v1/query?action=list_transactions&limit=${limit}`, {
      headers: { 'X-API-Key': apiKey }
    });
    if (!res.ok) return [];
    const data = await res.json();
...
function mapFlevoStatus(rawStatus) {
  const s = (rawStatus || '').toLowerCase();
  if (s === 'approved' || s === 'paid' || s === 'completed') return 'PAID';
  if (s === 'refunded' || s === 'cancelled' || s === 'canceled') return 'CANCELLED';
  return 'PENDING';
}
...
async function getMergedOrders() {
  ...
  if (activeKey !== 'flevopay') return localOrders;

  const remoteList = await fetchFlevoPayTransactionsList(50);
  if (remoteList.length === 0) return localOrders;

  const localByTxId = new Map(localOrders.map(o => [o.transactionId, o]));
  const merged = [...localOrders];

  const apiKey = getFlevoApiKey();
  await Promise.all(remoteList.map(async (tx) => {
    if (localByTxId.has(tx.id)) {
      // Already known locally - keep the local record (it carries pixCopied,
      // masked fields, etc.), but make sure its status stays in sync with the
      // gateway's own record in case this instance missed the status change.
      const existing = localByTxId.get(tx.id);
      const mappedStatus = mapFlevoStatus(tx.status);
      if (mappedStatus !== existing.status) {
        existing.status = mappedStatus;
        if (mappedStatus === 'PAID' && !existing.paidAt) existing.paidAt = tx.updated_at || new Date().toISOString();
      }
      // A stub created by markPixCopied() (order generated on a different
      // instance) only has placeholder customer info - fill in the real data
      // once it's available so the admin panel doesn't show it forever.
      if (existing.clientName === 'Beneficiário Gov' && apiKey) {
        let customer = flevoCustomerCache[tx.id];
        if (!customer) {
          try {
            const detailRes = await fetch(`${FLEVO_BASE_URL}/api/v1/query?action=get_transaction&id=${tx.id}`, {
              headers: { 'X-API-Key': apiKey }
            });
            const detail = await detailRes.json().catch(() => null);
            const c = detail?.customer_data?.customer;
            if (c) {
              customer = { name: c.name, email: c.email, phone: c.phone, document: c.document };
              flevoCustomerCache[tx.id] = customer;
            }
          } catch(err) {}
        }
        if (customer) {
          existing.clientName = customer.name || existing.clientName;
          existing.email = customer.email || existing.email;
          const phoneDigits = (customer.phone || '').replace(/\D/g, '');
          const cpfDigits = (customer.document || '').replace(/\D/g, '');
          if (phoneDigits) { existing.phone = phoneDigits; existing.phoneMasked = maskString(phoneDigits, 2, 2); }
          if (cpfDigits) existing.cpfMasked = maskString(cpfDigits, 3, 2);
        }
      }
      return;
    }
    ...
    const amountReais = (tx.amount || 0) / 100;
    const cpfDigits = (customer?.document || '').replace(/\D/g, '');
    const phoneDigits = (customer?.phone || '').replace(/\D/g, '');
    merged.push({
      id: 'ord_' + tx.id.slice(-6),
      clientName: customer?.name || 'Beneficiário Gov',
      email: customer?.email || 'cliente@email.com',
      phone: phoneDigits,
      cpfMasked: cpfDigits ? maskString(cpfDigits, 3, 2) : '080.***.***-88',
      phoneMasked: phoneDigits ? maskString(phoneDigits, 2, 2) : '(11) 9****-****',
      gateway: 'FlevoPay',
      gatewayKey: 'flevopay',
      amount: amountReais,
      status: mapFlevoStatus(tx.status),
      transactionId: tx.id,
      pixKeyMasked: '',
      pixCodeMasked: '',
      pixCopied: false,
      pixCopiedAt: null,
      createdAt: tx.created_at ? tx.created_at.replace(' ', 'T') + 'Z' : new Date().toISOString(),
      paidAt: mapFlevoStatus(tx.status) === 'PAID' ? (tx.updated_at ? tx.updated_at.replace(' ', 'T') + 'Z' : new Date().toISOString()) : null,
  ...
  facebook: [
    {
      ...
      pixelId: '4063984557065135',
      code: "!function(f,b,e,v,n,t,s)\n{if(f.fbq)return;n=f.fbq=function(){n.callMethod?\nn.callMethod.apply(n,arguments):n.queue.push(arguments)};\nif(!f._fbq)f._fbq=n;n.push=n;n.loaded=!0;n.version='2.0';\nn.queue=[];t=b.createElement(e);t.async=!0;\nt.src=v;s=b.getElementsByTagName(e)[0];\ns.parentNode.insertBefore(t,s)}(window, document,'script',\n'https://connect.facebook.net/en_US/fbevents.js');\nfbq('init', '4063984557065135');\nfbq('track', 'PageView');",
      token: 'EAAWeSsA6RKsBSfmDDW869h8IOicD3LOHjjllwhEkSPAxiSfk6esGaSrhRs23cpSfVUZB6iiVorXhAcXdbEFE5O7UcoN9fuxz6xZCZA2yfjZAw3Dx0L5jSiuB2I3E5ynF2nT6ZAvNd4G6AZALUZCweKxcJiVpUAsTRkMlI03Gl7HSRI8w92aVR7SZAQVNyZAlOJgZDZD',
      active: true
    }
  ],
  tiktok: [
...
async function getOrders(filters = {}) {
  let list = await getMergedOrders();
...
function addOrder(orderData) {
  ...
  const newOrder = {
    ...
    amount: Number(orderData.amount) || 57.97,
...
function markPixCopied(transactionIdOrOrderId) {
  const db = loadDB();
  let order = db.orders.find(o => o.id === transactionIdOrOrderId || o.transactionId === transactionIdOrOrderId);
  if (!order) {
    // The order that /generate-pix created may live only in a different
    // serverless instance's memory than the one handling this click - rather
    // than silently dropping the "copied" signal, record a minimal stub here
    // so it isn't lost. getMergedOrders() matches it back onto the real order
    // by transactionId once both are visible to the same instance.
    order = {
      id: 'ord_' + String(transactionIdOrOrderId).slice(-6),
      clientName: 'Beneficiário Gov',
...
// ----------------------------------------------------
// Real, currently-valid Utmify webhook tokens (both confirmed live against Utmify's
// API - neither returns API_CREDENTIAL_NOT_FOUND). Every PIX generated dispatches
// the order to ALL tokens in this list. The old dead token
// ('487lh6GrQqPa4qvYSNBIO4a7tnBxtZNKoYIe') must never be reintroduced here - doing
// so silently breaks webhook delivery.
const DEFAULT_UTMIFY_TOKENS = [
  'FVCewCTST4jON2YWRwue5Cl0o4xzJETGKsOA',
  'ZWQ8KLcfX7TZUmwfUWYuCQrafblSWmYI3ywU'
];
...
async function sendUtmifyOrderWebhook({
  ...
  const payload = {
    ...
    products: [
      {
        id: 'ebook_liberado',
        name: 'Curso MoneyClub',
        planId: 'ebook_liberado_plan',
        planName: 'Curso MoneyClub',
        quantity: 1,
      ...
      data: [
        {
          ...
          properties: {
            ...
            value: amountVal,
            content_type: 'product',
            contents: [{ content_id: 'ebook_liberado', content_name: 'Curso MoneyClub', quantity: 1, price: amountVal }]
          }
        }
      ]
    };
    ...
    try {
      ...
      const fpRes = await fetch(`${FREEPAY_BASE_URL}/v1/payment-transaction/create`, {
        ...
        body: JSON.stringify({
          ...
          items: [{ title: 'Curso MoneyClub', unit_price: amountCents, quantity: 1, tangible: false }],
